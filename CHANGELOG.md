...
- **Queued dispatch foundation** — public `Flow::dispatch()` API plus queueable after-commit `RunFlowJob` that carries the flow name, input, and `FlowExecutionOptions` into a worker-resolved `FlowEngine` under a per-dispatch cache lock. This is an early Macro Task 3 slice; retry/backoff policy, database queue integration, replay, and parallel compensation remain follow-up work.
