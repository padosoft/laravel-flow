...
- **Queued dispatch foundation** — public `Flow::dispatch()` API plus queueable after-commit `RunFlowJob` that carries the flow name, input, and `FlowExecutionOptions` into a worker-resolved `FlowEngine` under a shared per-dispatch cache lock. Duplicate deliveries that find the lock held are acknowledged as no-ops instead of failed attempts, and the process-local `array` cache store is rejected for queued execution. This is an early Macro Task 3 slice; retry/backoff policy, database queue integration, replay, and parallel compensation remain follow-up work.
