...
![Laravel Flow banner](resources/laravel-flow-banner.png)
...
- **Content-hash node result cache** — `#[Cacheable(ttl: ...)]` marks a graph node whose output is served from `flow_node_cache` on a content-hash hit of (node type, resolved inputs, node config), skipping the handler; the hash sorts dict keys but preserves list order (fan-in inputs are ordered), cache writes pass through the same payload redactor as every other table and are skipped entirely when redaction would alter the value (so a hit never diverges from a fresh run), and the cache is inert on dry-run / when persistence is off.
- **Graph saga compensation** — when a graph run fails, only completed nodes roll back; the default `reverse-order` strategy guarantees reverse-topological order (a downstream node undoes its side effects before anything it depended on), while the opt-in `parallel` strategy batches compensators concurrently and trades that ordering guarantee for throughput (same contract as v1 parallel compensation). A node opts in by implementing `CompensatableNode`, a compiled v1 step reuses its `compensateWith()` compensator (with the graph-adapter context: `stepOutputs` is empty, as during adapted execution), and the graph-level aggregate compensator always runs last. The run flips to `compensated` only when every intended compensator succeeded, a partial rollback keeps the failure state with `compensation_status='failed'`, and a throwing compensator never aborts the remaining rollback.
- **DAG dry-run plan + cost estimate** — `DryRunPlanner::plan($graph)` statically computes the execution plan as Kahn waves (wave 0 = roots, wave N = nodes whose predecessors all ran earlier — what could run concurrently) plus a cost estimate summed from per-node `#[Cost(estimate: ['tokens' => 1200, 'cents' => 3])]` hints (free-form numeric dimensions, advertised in the node catalog). Executes no handler, writes zero rows across every persistence table; the plan is optimistic (runtime self-skips are not predicted) and a node without a hint simply contributes no cost.
- **Approval gate node on graphs** — the built-in `flow.approval` node pauses a graph run exactly like v1's `approvalGate()` step (same hash-only one-time token, same `Flow::resume()`/`Flow::reject()` API — engine detection is automatic). On approval the paused node resolves to `succeeded` with the decision payload as its output and the queued coordinator re-advances the graph (a further approval gate, fan-out, or failure all behave exactly as in a normal queued run); on reject the node resolves to `failed`, which automatically triggers `GraphSaga` compensation of completed upstream nodes — no new code path, it composes with the existing saga. `GraphRunner::run()`'s result exposes the plain issued token the same way v1's `FlowRun` does.
- **Opt-in graph run/node broadcasting** — `laravel-flow.broadcasting.enabled` streams graph node transitions and an aggregate run-progress snapshot on a private, per-run channel (`{prefix}.run.{runId}`), Reverb/Pusher/etc.-compatible via whatever broadcast driver the host app already configures. The package only emits — it ships no channel authorization callback (the host app's `routes/channels.php` decides who may subscribe) and has zero broadcast dispatches when disabled (the default). Decoupled from `persistence.enabled` (broadcasting works even with persistence off); a dry run always stays silent.
- **Declarative trigger sources** — the companion [`padosoft/laravel-flow-connect`](https://github.com/padosoft/laravel-flow-connect) package starts a run from a `ScheduleTrigger` (cron entry registered with Laravel's own Scheduler), an `EventTrigger` (a host Laravel event maps to a run input; a mapping/registration failure is skipped and logged — it never causes a fatal error and never lets an exception escape into the host's own event-dispatch call stack), or an inbound signed `WebhookTrigger` (HMAC-SHA256 over `timestamp.body` + a timestamp-window replay guard, mirroring this package's own outbound webhook delivery scheme). All three implement one shared `Padosoft\LaravelFlow\Contracts\FlowTrigger` (`@api`) contract that lives in this core package, so a third-party trigger source can implement it too.
...
| Web admin UI | ✅ YES - companion admin panel available at [`padosoft/laravel-flow-admin`](https://github.com/padosoft/laravel-flow-admin) for runs, approvals, failures, outbox, and operational KPIs | ⚠️ PARTIAL - optional Waterline UI, tied to that package's durable workflow model | ❌ NO - no bundled admin panel | ✅ YES - Temporal Web UI for workflow executions and history | ✅ YES - AWS Console for state-machine executions and history |
...
| Content-hash node result caching | ✅ YES - opt-in `#[Cacheable]` graph nodes serve outputs from `flow_node_cache` on a content-hash hit of (node type, resolved inputs, node config), with per-node TTL and redaction-gated writes that are skipped whenever redaction would alter the value | ⚠️ PARTIAL - durable engines memoize activity results in event history for deterministic REPLAY within an execution, not a cross-execution content-addressed result cache | ❌ NO - the state-machine component has no result-cache concept | ⚠️ PARTIAL - activity results are recorded in history for replay determinism; cross-execution result caching keyed by input is app-implemented | ❌ NO - no native input-keyed result cache; implement via a Lambda + external store (e.g. DynamoDB/S3) |
| Graph-level saga compensation (DAG) | ✅ YES - on graph failure only completed nodes roll back, in reverse-topological order by default (the opt-in `parallel` batching trades ordering for concurrency), via a `CompensatableNode` handler capability, per-node v1 compensators on compiled steps, and an always-last graph-level aggregate compensator; the run is marked `compensated` only on a full rollback | ⚠️ PARTIAL - saga/compensation is written imperatively inside workflow code; no declarative per-node compensation over a DAG | ❌ NO - the state-machine component has no compensation concept | ⚠️ PARTIAL - the saga pattern is a documented idiom (compensation stack in workflow code, e.g. the Saga class in the Java SDK), not a declarative per-node DAG capability | ⚠️ PARTIAL - rollback is modeled manually with `Catch` + cleanup states; no automatic reverse-topological compensation of completed states |
| Pre-execution plan + cost estimate (dry-run) | ✅ YES - `DryRunPlanner::plan()` statically returns the wave-by-wave execution plan plus a cost estimate summed from per-node `#[Cost]` hints, executing nothing and writing zero rows | ❌ NO - no documented static plan/cost preview of a workflow before execution | ❌ NO - the state-machine component has no plan/cost concept | ❌ NO - no native pre-execution plan or cost estimate for a workflow (history/replay is post-hoc) | ⚠️ PARTIAL - state machines can be validated/rendered (e.g. Workflow Studio), but there is no native per-state cost-estimate aggregation |
| Approval gate as a declarative DAG node | ✅ YES - the built-in `flow.approval` graph node pauses a run with the same hashed one-time token and `Flow::resume()`/`Flow::reject()` API as the v1 step gate; approval re-advances the queued coordinator (further gates/fan-out behave as in a normal run), reject auto-triggers `GraphSaga` compensation of completed upstream nodes | ⚠️ PARTIAL - model manually as a long-running workflow/activity awaiting a signal; no declarative per-node pause primitive over a DAG | ❌ NO - the state-machine component has no DAG execution model or pause-for-approval primitive | ⚠️ PARTIAL - signals/`await_condition` are the idiom for human-in-the-loop, not a declarative per-node pause state | ✅ YES - a `Task` state's `.waitForTaskToken` callback pattern is precisely a per-node (per-state) pause primitive in their state-machine DAG |
| Opt-in live broadcasting of run/node progress | ✅ YES - `laravel-flow.broadcasting.enabled` streams node transitions + an aggregate progress snapshot on a private per-run channel over whatever driver (Reverb/Pusher/etc.) the host app already configures; package only emits, zero dispatches when disabled | ❌ NO - not documented as a built-in capability; a Laravel Echo integration would be app-assembled on top of the package's own events | ⚠️ PARTIAL - the component dispatches Symfony EventDispatcher transition events (guard/enter/completed); wiring those to a browser (e.g. via Mercure) is entirely app-implemented, not part of the component | ❌ NO - no native WebSocket/browser broadcast; the Web UI polls workflow history over gRPC/HTTP, and pushing live updates to a browser is an app-assembled pattern | ❌ NO - no native WebSocket broadcast; EventBridge can react to execution state-change events, but wiring that to a browser (e.g. via AppSync) is an assembled solution, not a native feature |
| Declarative trigger sources (schedule / event / webhook) | ✅ YES - the companion `padosoft/laravel-flow-connect` package ships three trigger types: `ScheduleTrigger` (a cron entry registered with Laravel's own Scheduler; malformed entries are skipped and logged, never fatal); `EventTrigger` (a host Laravel event maps to a run; a mapping failure never propagates into the host's own event-dispatch call stack); and an inbound signed `WebhookTrigger` (HMAC-SHA256 + timestamp-window replay protection, mirroring the package's own outbound webhook delivery scheme). All three implement one shared, `@api` `FlowTrigger` contract | ⚠️ PARTIAL - the sample app demonstrates a built-in webhook command that starts a workflow; schedule support exists at the durable-workflow server level, not confirmed as a declarative per-workflow package primitive; no documented host-Laravel-event trigger | ❌ NO - the Workflow component itself has no built-in trigger support; Symfony's separate Scheduler and Webhook components exist but require manually wiring an event subscriber to drive a transition | ⚠️ PARTIAL - first-class Schedules natively start a Workflow Execution on a cron/interval; no built-in signed inbound webhook - an app must implement its own HTTP endpoint calling the Temporal client to start or signal a workflow | ⚠️ PARTIAL - EventBridge Scheduler/Rules can start a state machine execution on a schedule or in response to an AWS event, but this is an external AWS service integration, not a package-native trigger; no built-in signed inbound webhook (requires an assembled API Gateway + Lambda solution) |
...
| Per-node retry / backoff / dead-letter | ✅ YES - `#[Retry(tries, backoff, timeout)]` per graph node (attribute or per-placement `config['retry']` override); an exhausted real retry budget (`tries > 1`) drives the node to a `dead_letter` state while a single failed attempt stays `failed` | ✅ YES - Temporal-style activity retry policies (max attempts, backoff) | ❌ NO - the state-machine component has no retry/backoff model (retry is app/Messenger-level) | ✅ YES - first-class activity `RetryPolicy` (initial interval, backoff coefficient, max attempts, non-retryable errors) | ✅ YES - per-state `Retry`/`Catch` with interval, backoff rate, and max attempts |
...
| Parallel DAG (graph) execution on the queue | ✅ YES - `Flow::dispatchGraph()` runs a node graph on the Laravel queue; a coordinator claims ready nodes under a per-run row lock and fans independent nodes out to per-node jobs, with lock-TTL + queue-retry recovery and no external service | ✅ YES - durable workflows execute activities on queue workers, including parallel activities | ❌ NO - synchronous state machine; async is app/Messenger-level, not a DAG executor | ✅ YES - worker-based durable execution with parallel activities / child workflows | ✅ YES - managed orchestration with a `Parallel` state |
| Declarative fan-out / map / sub-flows | ✅ YES - built-in `flow.subflow` / `flow.foreach` / `flow.map` nodes fan out over items with `maxConcurrency` and an ordered, exactly-once join (sequential batches on the sync runner, real capped concurrency on the queued runner) | ⚠️ PARTIAL - parallel activities and child workflows are expressible in workflow code, not as declarative fan-out nodes | ❌ NO - no fan-out or sub-workflow concept | ⚠️ PARTIAL - child workflows and parallel activities are first-class primitives but still code-driven, not declarative fan-out nodes | ✅ YES - first-class `Map` (incl. distributed) and `Parallel` states with concurrency control |
...
Competitor snapshot checked against Durable Workflow, Symfony Workflow, Temporal, and AWS Step Functions documentation on 2026-05-05; the content-hash node-cache row was added 2026-07-10, the graph-level saga, pre-execution plan/cost, and approval-gate-as-a-DAG-node rows on 2026-07-12, and the opt-in live broadcasting and declarative-trigger-sources rows on 2026-07-13, all against the same systems' documented execution/replay, saga-pattern, tooling, human-in-the-loop/callback, real-time-notification, and scheduling/eventing models. `laravel-flow` is **deliberately positioned** as the lightest Laravel-native dependency in the table. If you already run Temporal or AWS Step Functions and need their queue/replay/redrive guarantees today, use them. If you want saga semantics, dry-run, business-impact projection, opt-in persistence, signed webhook delivery, headless dashboard contracts, and a companion web admin UI inside an existing Laravel app, this is the package.
