...
- **Graph saga compensation** — when a graph run fails, only completed nodes roll back, in reverse-topological order (a downstream node undoes its side effects before anything it depended on); a node opts in by implementing `CompensatableNode`, a compiled v1 step reuses its `compensateWith()` compensator unchanged, and the graph-level aggregate compensator always runs last. Reuses the v1 `compensation_strategy` config (`reverse-order` default, opt-in `parallel` batching); the run flips to `compensated` only when every intended compensator succeeded, a partial rollback keeps the failure state with `compensation_status='failed'`, and a throwing compensator never aborts the remaining rollback.
