...
- If a queued flow run completes but the dispatch completion marker cannot be written, keep the per-dispatch lock until TTL instead of releasing it immediately; mark the queue job failed when a queue job context exists, then throw the marker error so sync dispatch surfaces the operational failure too.
...
- Do not accept Laravel's `array` cache store for real queued workers: it implements the lock provider contract, but the lock is process-local and cannot protect multiple queue workers. Keep it allowed for the `sync` queue driver so local inline dispatch remains ergonomic.
