...
- Check that a pending approval token still belongs to a `paused` run before consuming it; stale pending tokens from already failed/compensated runs must not be marked approved/rejected.
