...
- Queue job classes need property defaults, not only promoted readonly constructor properties, when adding serialized fields; in-flight jobs created before deployment must deserialize with sane defaults instead of uninitialized typed properties.
