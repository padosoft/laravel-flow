...
- If a queued flow run completes but the dispatch completion marker cannot be written, keep the per-dispatch lock until TTL instead of releasing it immediately; the failed marker is now an operational error and the TTL is the last duplicate-execution guard.
