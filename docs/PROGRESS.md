...
| Macro Task 3 - v0.2 queues/replay | Started with a queue dispatch foundation: `Flow::dispatch()` validates and queues an after-commit `RunFlowJob` carrying flow name, input, execution options, and a per-dispatch lock id; queued jobs release lock-held duplicates for a later attempt, no-op completed duplicates, and reject process-local `array` locks outside the `sync` queue driver. Retry/backoff, database-queue integration, replay, and parallel compensation remain follow-up slices. |
...
- `composer validate --strict --no-check-publish`
- `composer quality` => Pint format test, PHPStan, Unit 115 tests / 487 assertions, Architecture 2 tests / 7 assertions
