...
| Macro Task 4 - approval resume/reject API hardening | Active subtask on branch `task/approval-resume-api` with PR #29 into `task/v03-approval-webhooks`. Latest verified head `c1cf1f189ce92f65905c6ee2d9e0b68dd9ea6d68` was CI-green on PHP 8.3/8.4 and Copilot review was requested on that head. The slice now hardens approval resume/reject against downstream token reissue write failures, preserves running duplicates while downstream work is still in flight, keeps persisted completed downstream steps finishable, and keeps `docs/LESSON.md` / README comparison wording aligned. |
