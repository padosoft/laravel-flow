...
- Approve/reject by token HASH (`@api`): `FlowEngine::resumeByHash(string $tokenHash, array $payload = [], array $actor = []): FlowRun` and `rejectByHash(...)` (+ `Flow::resumeByHash`/`rejectByHash`) decide a persisted approval gate by the stored token HASH — the form a companion dashboard holds, since plain approval tokens are never recoverable from storage. They are byte-for-byte equivalent to `resume($plainToken)`/`reject($plainToken)` for the same approval: the entry points hash a trimmed plain token, the by-hash variants take the hash directly, and the shared decision path (cache-lock keyed by RUN id, token-consume, run continuation for both the v1 and graph engines) is otherwise unchanged. `ApprovalTokenManager` (`@api`) gains hash-native `findByHash`/`approveForRunStatusByHash`/`rejectForRunStatusByHash` (its existing `find`/`consume` now delegate through them). To give a dashboard the hash to act on, `Dashboard\ApprovalSummary` gains a trailing `?string $tokenHash = null` read property (additive — defaulted, so existing positional construction is unaffected) populated from `flow_approvals.token_hash`; `FlowDashboardReadModel::pendingApprovals()`/`listApprovals()` now surface it. A dashboard gates via `DashboardActionAuthorizer::canApproveByToken(string $tokenHash, …)`/`canRejectByToken(...)` — already hash-based — then calls `resumeByHash`/`rejectByHash`.
- Programmatic run replay (`@api`): `FlowEngine::replay(string $runId, ?FlowExecutionOptions $options = null): FlowRun` (+ `Flow::replay`) replays a TERMINAL persisted run as a new linked run — re-executing the run's definition with its recorded input and returning the NEW `FlowRun` (linked via `replayedFromRunId`). A pinned graph run re-executes its EXACT stored graph version (`DefinitionRepository::find($name, $version)`); a legacy run re-executes the currently-registered definition. Requires persistence enabled. `replayedFromRunId` is always forced to the source run id and the caller's (or the source run's) `correlationId` is used; a caller-supplied `idempotencyKey` is deliberately IGNORED so a replay is always a NEW linked run (honoring it could resolve to a differently-linked existing run on the legacy path, or hit the `flow_runs.idempotency_key` unique constraint on the pinned-graph path). Throws `FlowExecutionException` for a missing / non-terminal / non-array-input run, an **unpinned graph run** (engine `graph` with no stored version — it is NOT reinterpreted as a legacy flow), an unregistered definition, or an unloadable stored graph. It performs the same core replay as the existing `flow:replay` Artisan command (which stays unchanged) but is NOT a drop-in equivalent: the programmatic method additionally rejects unpinned graph runs, supports a `$options` correlation override, and does not emit the command's console definition-drift warnings. NOTE: `PublicApiContractTest` now pins `replay` on `FlowEngine` — the prior "replay is command-only" note is superseded.
- Run cancellation (`@api`): `FlowEngine::cancel(string $runId, array $actor = []): FlowRun` (+ `Flow::cancel`) atomically transitions a non-terminal run to `aborted` and moves every still-active node to a transition-legal terminal state (a `pending` node → `skipped`, a `running`/`paused` node → `failed`; the `NodeState` enum surface is unchanged — no new "cancelled" case). It is idempotent: cancelling an already-terminal run — or losing the run-status compare-and-set to a concurrent completion — returns the run's CURRENT `FlowRun` state unchanged rather than forcing it. Requires persistence enabled (throws `FlowExecutionException` otherwise). `$actor` is reserved for future audit attribution (no actor-scoped row is written today). A node cancelled this way lands on `failed` (the `NodeState` enum has no "cancelled" case) but is stamped `error_class = 'FlowRunCancelled'` / `error_message = 'Run was cancelled.'` so consumers (`FlowRun::results()`, the dashboard) can tell it apart from a genuine handler failure. `Contracts\RunNodeRepository` gains one method — `terminate(string $runId, string $nodeId, string $expectedStatus, string $newStatus, DateTimeInterface $finishedAt, ?int $durationMs, ?string $errorClass = null, ?string $errorMessage = null): bool` — a compare-and-set that moves a still-active node to a terminal state ONLY when it is still in the expected status (so a node a concurrent queued job just completed is never clobbered). Custom `RunNodeRepository` implementers must add it (same posture as the earlier `claim`/`releaseClaim`/`states` additions). Coverage/limitations: (a) only PERSISTED node rows are terminated — a queued graph run pre-seeds a `pending` row per node (all flip to `skipped`), but a synchronously-executed run that paused mid-flight has no rows for not-yet-reached downstream nodes, so those are left un-rowed; (b) an in-flight queued node job may still write its own row afterward, but the run stays terminal; (c) cancel does NOT recompute the `flow_runs` node-count columns nor emit a broadcast settle-point snapshot — a subscribed dashboard learns of the abort on its next poll; (d) cancelling a run paused at an approval gate NOW expires its still-pending `flow_approvals` rows in the same transaction as the abort (they flip to `expired` with `decided_at` stamped), so a leaked or retried token can never decide an approval against a dead run — backed by a new `Contracts\ApprovalRepository` method `expirePendingForRun(string $runId, DateTimeInterface $decidedAt): int` (`@api`; returns how many rows were expired, a bulk compare-and-set on `status = 'pending'` keyed by run id — idempotent 0 when there are none). Custom `ApprovalRepository` implementers must add it (same additive posture as `terminate` on `RunNodeRepository`).
...
- Distinct persistence-outage exception (`@api`): new `Exceptions\PersistenceUnavailableException extends FlowExecutionException`, raised by the DB-backed mutation seams — `FlowEngine::cancel()`, `replay()`, `redeliverWebhook()` — when their underlying query hits a driver error (tables not migrated / connection unreachable), instead of the plain `FlowExecutionException` they raised before. It lets a caller (e.g. a companion dashboard) tell an INFRASTRUCTURE outage (retry with backoff / HTTP 503) apart from an ordinary state conflict (HTTP 409) that these seams also express as `FlowExecutionException`. Fully backward compatible: it IS-A `FlowExecutionException`, so existing `catch (FlowExecutionException)` handlers are unaffected; and the pre-existing `Exceptions\ApprovalPersistenceException` now extends `PersistenceUnavailableException` (still transitively a `FlowExecutionException`), so a caller can catch the single `PersistenceUnavailableException` parent to cover every persistence outage — approval decisions included.
