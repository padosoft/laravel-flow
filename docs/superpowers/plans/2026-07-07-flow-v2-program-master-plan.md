...
**Architecture:** Evolve `padosoft/laravel-flow` in-place (approach A): typed node contracts → versioned graph definitions → event-driven graph executor → real-time/triggers → Studio (React Flow) → AI pack (LLM/MCP/agent/advisor) → v2.0 release. Satellite packages `laravel-flow-connect` and `laravel-flow-ai` bootstrap in Macros D and F.
...
1. TDD followed: failing test written and observed failing BEFORE implementation.
2. `composer quality` green (`pint --test` + `phpstan` + Unit + Architecture + Contract suites).
3. If `@api` surface touched: `tests/Contract` updated in the same commit.

### G2 — PR Gate (before each subtask PR merges)
1. All G1 gates green on the branch head.
2. Pre-push self-review skill run on the diff; test-count-readme-sync skill run if tests/README counts changed.
3. GitHub Copilot Code Review requested; loop until all actionable comments resolved (copilot-pr-review-loop skill).
...
| D-PR2 | `laravel-flow-connect` package bootstrap (composer, CI, `composer quality` parity, trigger contracts consumed from core) | Green CI on empty-but-wired package; contracts pinned |
...
- **Branch:** in `laravel-flow-admin` repo (`task/v2e-studio`) · **Depends on:** B, D (core released as v2-dev tag or path repo)
- **Objective:** Visual composer (React Flow island) + working operating console (spec §5), plus admin hygiene fixes.
...
**Macro E acceptance:** full loop demo — compose a flow on canvas, publish, run, watch it live, approve its gate, see costs; all mutations authz-gated; admin README truthful.

## Macro F — AI Pack

- **Branch:** new repo `padosoft/laravel-flow-ai` (`task/v2f-ai-pack`) · **Depends on:** C (E for advisor/builder UI subtasks)
- **Objective:** Agentic layer (spec §4): LLM node, guardrails, MCP client + server (flow-as-tool), bounded agent node, AI flow builder, **Flow Advisor** (suggest/improve, code + CLI + UI).
...
| F-PR9 | Advisor + builder UI in Studio (admin repo): suggestions inbox, "Improve this flow" visual diff on canvas, accept→creates draft / dismiss | E2E: accept creates draft version identical to advisor payload; dismiss persists; authz-gated |

**Macro F acceptance:** an external MCP client lists and invokes an opt-in flow, hitting a human approval; the LLM node self-repairs a schema violation; `flow:improve` on a fixture-history flow proposes a retry/cache improvement rendered as canvas diff; all with zero real network in CI.
...
| G-PR4 | Final audits: contract suite, test-count README sync, CHANGELOGs; tag `v2.0.0` (core) + initial tags for connect/ai; GitHub releases | `composer quality` green on all repos; release checklist from v1.0.0 process reused |

**Macro G acceptance = program done:** v2.0.0 published with truthful docs, green CI on the full matrix, and the Studio/AI demos reproducible from README instructions.
