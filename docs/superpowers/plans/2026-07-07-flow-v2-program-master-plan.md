...
- **v1 public API is preserved; v1 internal persistence is free to evolve** (user decision, 2026-07-09). The v1 fluent builder (`Flow::define()->step()->…->register()`), the engine's public execution methods, and every `@api` class users write against remain backward-compatible; any `@api` change follows the normal major process (mirrored in `tests/Contract` + `docs/UPGRADE.md`), and v1 handlers stay adapter-supported. But v1's *internal* persistence schema and `@internal` implementation (`flow_steps` and its Models/migrations, internal persistence namespaces) MAY be redesigned, migrated, or retired in the v2 major release: there is exactly one downstream consumer and it controls its own upgrade timing. Concretely, v2 unifies step and node persistence into a single `flow_run_nodes` table written by both the v1 linear engine and the graph executor, with a documented data-migration path from `flow_steps`; v1 execution semantics (step/compensation ordering, approval resume) remain observably identical — only the persistence target changes.
...
- **Persistence unification (resolved 2026-07-09, per the relaxed v1-internal-persistence constraint above):** C-PR1 also unifies per-node persistence into a single `flow_run_nodes` table written by BOTH the v1 linear engine and the new graph executor, retiring `flow_steps` via a data migration. A v1 step is already modelled as a `legacy.step` graph node since Macro B (`FlowDefinition::toGraphDefinition()`), so this is the natural data model rather than a bolt-on second table; it also gives Macro D one progress-snapshot shape and Macro E one console row type. v1 EXECUTION semantics (compensation ordering, approval resume, sync behavior) stay observably unchanged — only the persistence target moves. Full detail: `docs/superpowers/plans/2026-07-08-macro-c-graph-executor.md`.

| Subtask PR | Deliverable | Gate criteria |
|---|---|---|
| C-PR1 | `NodeState`/`RunState` state machines (adds `blocked`, `invalid_input`, `partially_succeeded`, `dead_letter`) + transition guards; **unified `flow_run_nodes` persistence** (v1 engine rewired onto it, `flow_steps` retired via data migration) | Exhaustive legal/illegal transition tests; full v1 regression suite green against the unified table with behavior unchanged; `flow_steps` retirement migration idempotent |
