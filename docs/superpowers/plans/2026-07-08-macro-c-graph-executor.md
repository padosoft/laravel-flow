...
- `FlowEngine` (`@internal`): `execute(string $name, array $input, ?FlowExecutionOptions $options = null): FlowRun`; `registerDefinition(FlowDefinition $definition): void`. **Run status values are bare string literals**: `'succeeded'`, `'failed'`, `'skipped'`, `'paused'` (no `blocked`/`invalid_input`/`partially_succeeded`/`dead_letter` yet). Step status literals: `'succeeded'`, `'failed'`, `'skipped'`, `'paused'`. Step persistence flows through `FlowStore` → `StepRunRepository` (VERIFY the exact `persistStepStarted`/`persistStepFinished` → `steps()->createOrUpdate()`/`upsert()` seam — this is the code that moves onto `flow_run_nodes` in Task 3).
...
**Interfaces:**
- `enum NodeState: string` (`@api`) cases: `Pending='pending'`, `Running='running'`, `Completed='completed'`, `Failed='failed'`, `Skipped='skipped'`, `Blocked='blocked'`, `InvalidInput='invalid_input'`, `DeadLetter='dead_letter'`. Methods `isTerminal(): bool`, `canTransitionTo(self $to): bool`, `transitionTo(self $to): self` (throws when illegal).
- `enum RunState: string` (`@api`) cases: `Pending`, `Running`, `Paused`, `Succeeded`, `PartiallySucceeded='partially_succeeded'`, `Failed`, `Compensated`, `Aborted`, `DeadLetter='dead_letter'`. Same three methods. **These enums are the single source of truth for BOTH engines' status strings** — v1's `'succeeded'`/`'failed'`/`'skipped'`/`'paused'` map onto `NodeState`/`RunState` values (all already present), so unification extends the vocabulary without changing v1's persisted status strings.
- `IllegalStateTransitionException extends \LogicException` (`@api`) — message `"Illegal {enum} transition [{from}] → [{to}]."`.

**Legal transition tables** (encode as `match` inside `canTransitionTo`). Pauses are a RUN concern — a node stays `Running` while its approval resolves; `Failed → Running` (retry) is the ONLY node resurrection edge.

```php
// NodeState::canTransitionTo
return match ($this) {
    self::Pending => in_array($to, [self::Running, self::Skipped, self::Blocked, self::InvalidInput], true),
    self::Running => in_array($to, [self::Completed, self::Failed, self::DeadLetter], true),
    self::Failed  => in_array($to, [self::Running, self::DeadLetter], true), // retry / give up
    self::Completed, self::Skipped, self::Blocked, self::InvalidInput, self::DeadLetter => false, // terminal
};
...
- [ ] **Step 1: Failing tests.** `NodeStateTest`: `test_terminal_states_report_terminal`, `test_legal_transitions_are_allowed` (data provider over every legal edge), `test_illegal_transitions_throw` (representative illegal set: `Completed→Running`, `Pending→Completed`, `Blocked→Running`, `DeadLetter→Running`), assert the message contains both values. Mirror `RunStateTest`.
- [ ] **Step 2: RED.** **Step 3: Implement** (Illuminate-free). **Step 4: GREEN** + extend the annotation sweep root list and pin every enum case value in `ExecutorApiContractTest` (`assertSame('blocked', NodeState::Blocked->value)` etc.). **Step 5:** `composer quality`; commit `feat(executor): node and run state machines with transition guards`.

**Edge cases the tests MUST cover:** `Failed → Running` retry is legal but `Completed → Running` is NOT (idempotency); `Blocked` has zero outgoing edges (poisoned-by-upstream marker, distinct from the deliberate `Skipped`); `InvalidInput` is terminal and distinct from `Failed`; every case is reachable-or-terminal (no orphan).
...
**Interface:** `final class ReadinessResolver` (`@api`): `resolve(GraphDefinition $graph, array $states): ReadinessDecision` (`$states` = `array<string, NodeState>`; absent nodes = `Pending`). Ready iff `Pending` AND every predecessor is `Completed`/`Skipped`. Blocked iff `Pending` AND ≥1 predecessor is `Failed`/`Blocked`/`InvalidInput`/`DeadLetter`. `allTerminal` = every node `isTerminal()`.
    ...
    foreach ($graph->topologicalOrder() as $id) {          // deterministic order
        ...
        foreach ($predecessors[$id] ?? [] as $p) {
            ...
            if (in_array($ps, [NodeState::Failed, NodeState::Blocked, NodeState::InvalidInput, NodeState::DeadLetter], true)) {
                $anyPoisoned = true;
            } elseif (! in_array($ps, [NodeState::Completed, NodeState::Skipped], true)) {
...
**`flow_node_cache` schema:** `id`, `content_hash` string(64) unique, `node_type`, `outputs` json, `business_impact` json nullable, `created_at`, optional `expires_at`. **`outputs` is stored UNREDACTED** — this is deliberately different from `flow_run_nodes`/`flow_audit`: the cache's job is to reproduce the EXACT value a cache miss would have produced, and a cache hit feeds that value to downstream nodes as real execution data, not as an audit record. Storing the redaction-gate's placeholder here would silently swap a downstream node's real input for `[redacted]` on a cache hit while a cache miss would pass the genuine value — a correctness bug, not a privacy improvement (the redaction gate's whole point is protecting data AT REST in durable audit trails; `flow_node_cache` is ephemeral execution-input reuse, same trust boundary as the in-memory `NodeContext`/`NodeResult` payloads that already carry unredacted values during a run). Caching stays opt-in per node via the attribute's presence (same pattern as `#[Retry]`/`#[Cost]`) — a node handling sensitive output simply never carries `#[Cacheable]`. **ContentHasher (reuse the Macro B checksum lesson CAREFULLY — verify which part actually applies):** sha256 over a canonicalized array `['type'=>.., 'inputs'=>.., 'config'=>..]`; recursively sort ASSOCIATIVE/dict keys only. Do **NOT** sort PHP lists here, unlike `GraphSerializer::checksum()`'s graph-structure hashing — that lesson is about graph structure (where node/connection list order is genuinely non-semantic), but node inputs are NOT: C-PR2 defines `multiple` ports as ORDERED lists (coalesced by topological index, explicitly pinned), so `[a, b]` and `[b, a]` must hash differently or a cache hit could return the wrong node's output for a differently-ordered-but-same-set input. `#[Cacheable(ttl: ?int = null)]`.

- [ ] TDD: `ContentHasherTest`: `test_hash_stable_across_input_key_order`, `test_hash_differs_on_value_change`, `test_list_order_semantics_pinned` (document + pin whether input list order is significant — for node inputs it IS: equal lists in equal order hash equal, different order differs). `NodeCacheTest`: `test_miss_then_store_then_hit`, `test_hit_returns_cached_outputs_without_running_handler` (invocation-count = 0 on hit), `test_cache_hit_returns_the_genuine_output_value_even_when_persistence_redaction_is_enabled` (a cached output containing a key the redaction gate would normally mask must come back UNREDACTED on hit — assert it does NOT equal the configured redaction placeholder — proving cache reuse never silently substitutes a redacted value for real execution data), `test_dry_run_never_reads_or_writes_cache` (zero cache rows), `test_ttl_expiry`. Commit `feat(executor): content-hash node cache with cacheable attribute`; **close C-PR7**, branch `task/v2c-08-graph-saga`.

**C-PR7 gate criteria:** hit/miss tests; dry-run cache-inert (zero reads/writes asserted); hash stable across input key order; cache hit records `cache_hit` on the node run and skips the handler.
