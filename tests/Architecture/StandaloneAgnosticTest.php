...
final class StandaloneAgnosticTest extends TestCase
{
    ...
    public function test_src_directory_contains_no_forbidden_substring(): void
    {
        ...
        $this->assertSame([], $offenders, sprintf(
            "Forbidden substrings found in src/:\n%s\nlaravel-flow MUST stay standalone-agnostic — no AskMyDocs / sister-package leakage.",
            implode("\n", $offenders),
        ));
    }
    ...
    public function test_composer_json_does_not_require_a_sister_package(): void
    {
        ...
        $forbidden = ['lopadova/askmydocs', 'padosoft/askmydocs-pro', 'padosoft/laravel-patent-box-tracker'];

        foreach ($forbidden as $dep) {
            $this->assertArrayNotHasKey(
                $dep,
                $require,
                sprintf('laravel-flow must not require [%s] in production deps.', $dep),
            );
        }
    }
}
